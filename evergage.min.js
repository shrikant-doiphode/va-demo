...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "fedexcorp",
           dataset: "fedex_staging",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://fedexcorp.us-6.evergage.com",
           siteConfigVersion: "181"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableCrossDomainIframeCookies":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["UUID","customerId","emailAddress","profileId","sfmcContactKey"],"lastModified":1695407924480,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":true,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
const getCookieDomain = () => {
    ...
    if (currentDomain === "wwwtest.fedex.com") {
        return "wwwtest.fedex.com";
    }
}
...
SalesforceInteractions.init({
  ...
  // last rates shown
  function lastRatesShown() {
    // === Step 0: Load and parse rates from localStorage ===
    const rawRates = localStorage.getItem("rates");
    if (!rawRates) {
      console.warn("No rates key found in localStorage");
      return null;
    }

    let parsedRates;
    try {
      parsedRates = JSON.parse(rawRates);
    } catch (err) {
      console.error("Error parsing rates from localStorage:", err);
      return null;
    }

    const allRatesByDate = parsedRates?.rate?.rateList?.rates;
    if (!allRatesByDate || typeof allRatesByDate !== "object") {
      console.warn("No rate list found in the parsed data.");
      return null;
    }

    // === Step 1: Find the earliest date ===
    const dateKeys = Object.keys(allRatesByDate).filter(Boolean);
    if (!dateKeys.length) {
      console.warn("No valid date keys found in rates.");
      return null;
    }

    const earliestDate = dateKeys.sort((a, b) => new Date(a) - new Date(b))[0];

    const ratesOnEarliest = Array.isArray(allRatesByDate[earliestDate])
      ? allRatesByDate[earliestDate]
      : [];

    // === Step 2: Find the lowest totalNetCharge for that date ===
    let lowestRate = null;
    for (const rate of ratesOnEarliest) {
      const amount = rate?.pricing?.totalNetCharge?.amount;
      if (Number.isFinite(+amount)) {
        if (!lowestRate || +amount < +lowestRate.pricing.totalNetCharge.amount) {
          lowestRate = rate;
        }
      }
    }

    // === Step 3: Store lowest rate info ===
    const lowestRateInfo = lowestRate
      ? {
          serviceName: lowestRate.serviceName,
          amount: +lowestRate.pricing.totalNetCharge.amount,
          currency: lowestRate.pricing.totalNetCharge.currency,
          deliveryDate: earliestDate,
        }
      : null;

    // === Step 4: Find lowest RECOMMEND (totalNetFedExCharge, no currency) ===
    const lowestRecommend = (() => {
      try {
        const allRates = Object.values(allRatesByDate)
          .flatMap(r => (Array.isArray(r) ? r : []))
          .filter(r => r?.rateType === "RECOMMEND");

        const fedexCharges = allRates
          .map(r => r?.pricing?.totalNetFedExCharge)
          .filter(v => v && Number.isFinite(+v.amount));

        if (!fedexCharges.length) {
          console.log("No RECOMMEND-rates found.");
          return null;
        }

        const minCharge = fedexCharges.reduce((a, b) =>
          +b.amount < +a.amount ? b : a
        );

        console.log("Lowest RECOMMEND (totalNetFedExCharge):", +minCharge.amount);

        return +minCharge.amount; // numeric only, no currency
      } catch (err) {
        console.log("Error in lowestRecommend:", err?.message || err);
        return null;
      }
    })();

    // === Step 6: Return JSON with only existing keys ===
    const result = {};
    if (lowestRateInfo?.serviceName)  result.serviceName = lowestRateInfo.serviceName;
    if (lowestRateInfo?.amount != null) result.amount = lowestRateInfo.amount;
    if (lowestRateInfo?.currency)    result.currency = lowestRateInfo.currency;
    if (lowestRateInfo?.deliveryDate)result.deliveryDate = lowestRateInfo.deliveryDate;
    if (lowestRecommend != null)     result.lowestRecommend = lowestRecommend;

    return Object.keys(result).length ? result : null;
  }

  // ===== Fire TEST event when LMR code runs =====
  function fireTestLmrEvent() {
    const summary = lastRatesShown();

    // only fire if we actually computed something
    if (!summary) {
      console.warn("TEST- LMR not sent: no LMR summary available.");
      return;
    }
    ...
    SalesforceInteractions.sendEvent({
      ...
      user: {
        attributes: {
          // what you asked to send for testing
          lastRatesShown: nowIso,
          lastLmrSet: JSON.stringify(summary)
        }
      }
    });

    console.log("Sent MCP event: TEST- LMR", { lastRatesShown: nowIso, lastLmrSet: summary });
  }

  // Call this wherever “the code is fired”
  // Example: run immediately after init
  fireTestLmrEvent();
    ...
    function pageMatch(pagePathCheck1, pagePathCheck2, pagePathCheck3) {
        return new Promise((resolve, reject) => {
            ...
            const checkDataLayer = () => {
                ...
                // const pagePath3 = gdl?.params?.page?.pagePath3;
...
function sendMcpEvent() {
  ...
  // Helper: wait for element + return tracking ID text (SPA/Angular safe)
  function getTrackingID(timeoutMs = 10000) {
    const selector = '[data-test-id="outbound-master-tracking-number"]';
    ...
    return new Promise((resolve) => {
      ...
      // Observe DOM until it appears (Angular / SPA)
  ...
  if (eventName === 'ratesViewed') {
    ...
    const getRateObject = {
      event: {
        name: getRatePayload?.sender?.countryCode === 'US' && getRatePayload?.receiver?.countryCode === 'US'
          ? "mcpGetRate_US"
          : "mcpGetRate_INTL",
        sender: senderInfo,
    ...
    if (senderInfo != undefined && receiverInfo != undefined) {
      ...
      const lastLmrSet = lastRatesShown()?.lowestRecommend ? new Date().toISOString() : null;
      localStorage.setItem("LMRLastDateTime", lastLmrSet);
      ...
      SalesforceInteractions.sendEvent({
        ...
        user: {
          attributes: {
            ...
            lastRatesShown: lastRatesShown(),
            lastLmrSet: lastLmrSet
          }
        }
      });
    }
  ...
  // Label Confirmation + Tracking ID
  if (
    eventName === 'pageview' &&
    (
      pageName === 'fedex/apps/magp/shipment/finalized' ||
      (pagePath1 === 'lite' && pagePath2 === 'thankyou')
    )
  ) {
    getTrackingID().then((trackingID) => {
      SalesforceInteractions.sendEvent({
        interaction: {
      ...
      // Fail open: still send the event even if tracking ID extraction fails
      console.warn("Tracking ID extraction failed:", e);
    ...
    /********************************* Adobe GDL Campaign Stats *********************************/

    function mcpCampaignStatToDataLayer() {
        console.log('OnStatSend Listener Applied')
        ...
        document.addEventListener(SalesforceInteractions.mcis.CustomEvents.OnStatSend, (event) => {
            ...
            if (isThirdPartyLoaded && statType === "Impression") {
                console.log('Impression Event Triggered' + experienceName + " Stat Object: " + campaignType)
                window.FDX.GDL.push(['event:publish',
                    ['personalization', 'impression',
                        {
                            'personalization_id': campaignId,
                            'personalization_name': campaignName,
                            'personalization_variation_id': experienceId,
                            'personalization_variation_name': experienceName,
                            'personalization_tool': 'sfmc'
                        }
                    ]
                ])
            }
            else if (isThirdPartyLoaded && statType === "Clickthrough") {
                console.log('Clickthrough Event Triggered' + experienceName)
                window.FDX.GDL.push(['event:publish',
    ...
    let config = {
        global: {
            onActionEvent: (actionEvent) => {
                ...
                let email = ((JSON.parse(sessionStorage.getItem('userDetails') || '{}').userProfile || {}).userProfileAddress || {}).contact?.emailAddress || ''
                // let uuid = (JSON.parse(sessionStorage.getItem('userDetails') || '{}').userProfile || {}).uuid || ''
                if (email) {
                    let email = ((JSON.parse(sessionStorage.getItem('userDetails') || '{}').userProfile || {}).userProfileAddress || {}).contact?.emailAddress || ''
                    if (email && email != undefined && email != null && email != 'no') {
                        actionEvent.user = actionEvent.user || {}
                        actionEvent.user.identities = actionEvent.user.identities || {}
                ...
                const cookieName = 'control_group';
                let controlNum = getCookie(cookieName);

                if (!controlNum) {
                    controlNum = Math.floor(Math.random() * 100) + 1;
                    actionEvent.user = actionEvent.user || {}
                    actionEvent.user.attributes = actionEvent.user.attributes || {}
                    actionEvent.user.attributes.controlNum = controlNum
                    setCookie(cookieName, controlNum, 730);
                };

                // bronek _delete it... so that trigger resets
                const LMRnow = new Date();
                const LMRtwoHours = 2 * 60 * 60 * 1000;
                const LMRlastDateTime = localStorage.LMRLastDateTime;
                const LMRmcp = localStorage.LMRMCP;

                if (LMRlastDateTime) {
                    const LMRlastTime = new Date(LMRlastDateTime);
                    const isInvalid = isNaN(LMRlastTime);
                    const isExpired = !isInvalid && (LMRnow - LMRlastTime > LMRtwoHours);

                    if (isInvalid || isExpired) {
                        localStorage.removeItem("LMRLastDateTime");
                        actionEvent.user.attributes.lastLmrSet = null;
                    }
                } else if (!LMRmcp) {
                    localStorage.setItem("LMRMCP", LMRnow.toISOString());
                    actionEvent.user.attributes.lastLmrSet = null;
                }
            ...
            listeners: [
                SalesforceInteractions.listener('click', 'a[data-analytics="hero|Ship now"]', () => { //data analytics tag
    ...
    // File Claims — place this near the top of the identification section
    config.pageTypes.push({
        name: "File Claims",
        isMatch: () => {
            const path = location.pathname;

            // Read GDL synchronously if present (supports FDX.GDL or gdl)
            const p1 =
                (window.FDX?.GDL?.params?.page?.pagePath1) ??
                (window.gdl?.params?.page?.pagePath1);
            const p2 =
                (window.FDX?.GDL?.params?.page?.pagePath2) ??
                (window.gdl?.params?.page?.pagePath2);

            const gdlOK = (p1 === "customer-support" && /^(claims|file-claims)$/i.test(p2 || ""));

            // Locale prefix optional; supports /claims or /file-claims; slash or .html endings
            const rxOK = /\/(?:[a-z]{2}-[a-z]{2}\/)?customer-support\/(?:file-)?claims(?:\/|\.html|$)/i.test(path);

            const matched = gdlOK || rxOK;

            if (window.__evgDebug) {
                console.log("[Sitemap] File Claims isMatch?", { gdlOK, rxOK, matched, path, p1, p2 });
            }
            return matched;
        },

        interaction: { name: "View File Claims Page" },

        contentZones: [
            {
                name: "File claims hero",
                selector: '.fxg-landing-hero__background-image'
            },
            {
                name: "File claims subheader",
                selector: 'h2[aria-label="Get to know your role in the claims process"]'
            },
            {
                name: "File claims icon",
                selector: ".title_v1 h4.fxg-title.cc-aem-c-heading--h4"
            }
        ],
        ...
        listeners: [
            SalesforceInteractions.listener(
                "click",
                'a[data-analytics="hero|LOG IN TO START CLAIM"]',
                () => {
                    console.log("Click File Claims");
                    SalesforceInteractions.sendEvent({
                        interaction: { name: "Clicked File Claims Login" }
                    });
                }
            )
        ]
    ...
    config.pageTypes.push({
        ...
        contentZones: [
            {
                name: "Home Cheers Moments Header test",
                selector: 'h2[aria-label="Cheers to all of life’s special moments!"]'
            },

            {
                name: "Home Landing 2 Column",
                selector: '#container-c7a03791ff > div.experiencefragment > div > div > div.featured_offer_v2.aem-GridColumn.aem-GridColumn--default--12 > div'
            },

            {
                name: "Home Manage Header",
                selector: '#manage'
            },
            {
                name: "Home Breeze Into Summer",
                selector: 'h2[aria-label="Breeze into summer with these business boosters"]'
            },
    ...
    config.pageTypes.push({
        ...
        onActionEvent: (event) => {
            ...
            event.user.attributes.lastPageVisited = "Shipping";
            return event;
        },
        listeners: [
            SalesforceInteractions.listener('click', '[data-analytics="link|Calculate shipping costs"]', (e) => {
    ...
    config.pageTypes.push({
        name: "Shipping- schedule manage pickups",
        isMatch: () => pageMatch('shipping', 'schedule-manage-pickups'),
        interaction: {
            name: SalesforceInteractions.CatalogObjectInteractionName.ViewCatalogObject,
            catalogObject: {
                type: "Category",
                id: () => {
                    let id = 'shipping | schedule-manage-pickups'
                    return id
                },
            }
            ...
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "shipping - schedule-manage-pickups";
            return event;
        },
        contentZones: [
            ...
            {
                name: 'Shipping- schedule-manage-pickups',
                selector: 'body > div.fxg-main-content > div.root.responsivegrid > div > div > div > div.responsivegridshortparsys.responsivegrid.aem-GridColumn--default--none.aem-GridColumn.aem-GridColumn--default--10.aem-GridColumn--offset--default--1 > div > div > div:nth-child(23) > div'
            }
    ...
    config.pageTypes.push({
        name: "Overnight",
        isMatch: () => pageMatch('shipping', 'overnight'),
        interaction: {
            name: 'Overnight',
        },
        listeners: [
        ...
        contentZones: [
            {
                name: "Overnight Spacer",
                selector: 'body > div.fxg-main-content > div.root.responsivegrid > div > div > div > div.spacer.aem-GridColumn.aem-GridColumn--default--12 > div'
            },
        ]
    ...
    config.pageTypes.push({
        name: "International",
        isMatch: () => pageMatch("shipping-services", "international"),
        interaction: {
            name: SalesforceInteractions.CatalogObjectInteractionName.ViewCatalogObject,
            catalogObject: {
                type: "Category",
        ...
        listeners: [
            ...
            //     })
            // }),
            SalesforceInteractions.listener('click', 'a[data-analytics="foc|International 101"]', (e) => {
                SalesforceInteractions.sendEvent({
                    interaction: {
    ...
    config.pageTypes.push({
        ...
        interaction: {
            ...
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "Small Business";
            return event;
        },
        contentZones: [
            {
    ...
    config.pageTypes.push({
        ...
        interaction: {
            ...
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "tracking";
            return event;
        },
        contentZones: [
    ...
    //Tracking- Advanced
    config.pageTypes.push({
        name: "Tracking - Advanced",
        isMatch: () => pageMatch('tracking', 'advanced'),
        interaction: {
            name: SalesforceInteractions.CatalogObjectInteractionName.ViewCatalogObject,
            catalogObject: {
                type: "Category",
                id: () => {
                    let id = 'tracking | advanced'
                    return id
                },
            }
            ...
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "Tracking - Advanced";
            return event;
        },
        contentZones: [

            {
                name: 'Tracking Advanced - Access and Manage CMP',
                selector: '#container-a5a9a37560 > div:nth-child(6)'
            }
    ...
    config.pageTypes.push({
        name: "Manage Account",
        isMatch: () => pageMatch('manage-account'),
        interaction: {
            name: 'Manage Account',
        },
        onActionEvent: (event) => {
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "Manage Account";
            return event;
        },
        contentZones: [

            {
                name: 'Manage Account - Access and Manage CMP',
                selector: '#container-e939b8c53f'
            }
    ...
    config.pageTypes.push({
        name: "Billing Online",
        isMatch: () => pageMatch('billing-online'),
        interaction: {
            name: "Billing Online",
            ...
            event.user = event.user || {};
            event.user.attributes = event.user.attributes || {};
            event.user.attributes.lastPageVisited = "Billing Online";
            return event;
        },
        ...
        contentZones: [
            {
                name: "billing-online-billingtools and guidance",
                selector: "#container-a018ac4787 > div:nth-child(3)"
            },
        ]
    ...
    config.pageTypes.push({
        ...
        onActionEvent: (event) => {
            if (!window.location.pathname.split('/').includes('guest')) {
                event.user = event.user || {};
                event.user.attributes = event.user.attributes || {};
                event.user.attributes.isAccountHolder = true;
                return event;
        ...
        contentZones: [
            {
                name: "FedEx Ship Manager Main",
                selector: 'body > app-root > shell-wrapper > div > main-layout > ui-header'
            },
        ]
        // test bb                
    });
